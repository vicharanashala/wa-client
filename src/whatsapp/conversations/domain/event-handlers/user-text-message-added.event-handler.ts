...
export class UserTextMessageAddedHandler implements IEventHandler<UserMessageAddedEvent> {
  ...
  async handle(event: UserTextMessageAddedEvent): Promise<void> {
    ...
    let messages = toBaseMessages(conversation.messages);

    // Trim to last 10 messages to prevent token overload (Qwen3 skips tool calls with huge context)
    if (messages.length > 10) {
      this.logger.log(`[${event.phoneNumber}] Trimming message history from ${messages.length} to last 10 messages`);
